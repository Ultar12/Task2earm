...
const axios = require('axios'); // Added for Automatic Payouts
...
bot.on('message', async (msg) => {
    ...
    // --- FIX: DON'T DELETE ADMIN COMMANDS ---
    if (text.startsWith('/')) {
...
bot.onText(/\/stats/, async (msg) => {
    ...
    bot.sendMessage(msg.chat.id, `Total Users: ${totalUsersRes.rows[0].count}`);
});
...
bot.onText(/\/pending/, async (msg) => {
    ...
    try {
        const res = await pool.query(`
            SELECT t.id, t.chat_id, t.amount, u.username, u.bank_name, u.account_number 
            FROM transactions t 
            JOIN users u ON t.chat_id = u.chat_id 
            WHERE t.type = 'withdrawal' AND t.status = 'pending' 
            LIMIT 10`);

        if (res.rows.length === 0) return bot.sendMessage(chatId, "No pending withdrawals.");

        let m = "Pending Withdrawals:\n\n";
        res.rows.forEach(r => {
            m += `ID: ${r.id} | User: ${r.chat_id}\nAmt: ${r.amount} | Bank: ${r.bank_name}\nAcc: ${r.account_number}\n/approve ${r.id}\n\n`;
        });
        bot.sendMessage(chatId, m);
    } catch (e) { bot.sendMessage(chatId, "Error fetching pending."); }
});
...
bot.onText(/\/approve (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(txRes.rows[0].chat_id, "Your withdrawal request has been approved.").catch(()=>{});
...
bot.onText(/\/add (\d+) (\d+)/, async (msg, match) => {
    ...
    try {
        ...
        if (res.rows.length > 0) {
            await pool.query('INSERT INTO transactions (chat_id, type, amount, status) VALUES ($1, $2, $3, $4)', [targetUserId, 'admin_add', amount, 'completed']);
            bot.sendMessage(chatId, `Added ${amount} to ${targetUserId}. New balance: ${res.rows[0].balance}`);
        } else {
