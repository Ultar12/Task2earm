...
bot.on('message', async (msg) => {
    ...
    // --- FIX: STOP DELETION OF ADMIN COMMANDS ---
    if (text.startsWith('/')) {
        ...
        if (state.step === 'AWAITING_WITHDRAW_AMOUNT') {
            ...
            try {
                ...
                if (flwResp.data.status === "success") {
                    ...
                    replaceMessage(chatId, userId, `Success! ${amount} NGN sent to your account.`, mainMenu);
                } else { throw new Error("FLW fail"); }
            } catch (err) {
                replaceMessage(chatId, userId, "Forwarded to admin for review.", mainMenu);
                if (txId) {
                    bot.sendMessage(process.env.ADMIN_ID, `Payout Fail User ${userId}\nAmt: ${amount}\nApprove: /approve ${txId}`, {
    ...
    if (text === 'Task') {
        ...
        if (parseInt(refToday.rows[0].count) < 1) return replaceMessage(chatId, userId, "Need 1 verified referral today.", mainMenu);
        ...
        replaceMessage(chatId, userId, `Withdraw amount:`, cancelMenu);
        ...
        bot.getMe().then(async (b) => {
            ...
            replaceMessage(chatId, userId, `Link: https://t.me/${b.username}?start=${userId}`, mainMenu);
        });
    }
});
...
bot.on('callback_query', async (q) => {
    ...
    if (data === 'verify_join') {
        if (await checkMembership(userId, q.from.username)) {
            ...
            bot.answerCallbackQuery(q.id, { text: "Verified!" }); return replaceMessage(chatId, userId, "Verified! Bonus added.", mainMenu);
...
bot.onText(/\/pending/, async (msg) => {
    if (msg.from.id.toString() !== process.env.ADMIN_ID) return;
    const res = await pool.query(`SELECT t.id, t.chat_id, t.amount FROM transactions t WHERE t.type = 'withdrawal' AND t.status = 'pending' LIMIT 10`);
    if (res.rows.length === 0) return bot.sendMessage(msg.chat.id, "No pending.");
    bot.sendMessage(msg.chat.id, `Pending IDs: ${res.rows.map(r => r.id).join(', ')}`);
});

bot.onText(/\/add (\d+) (\d+)/, async (msg, match) => {
    if (msg.from.id.toString() !== process.env.ADMIN_ID) return;
    const target = match[1]; const amount = parseInt(match[2]);
    await pool.query('UPDATE users SET balance = balance + $1 WHERE chat_id = $2', [amount, target]);
    bot.sendMessage(msg.chat.id, `Added ${amount} to ${target}`);
});
...
bot.onText(/\/approve (\d+)/, async (msg, match) => {
    ...
    const txId = match[1];
    await pool.query("UPDATE transactions SET status = 'completed' WHERE id = $1", [txId]);
    bot.sendMessage(msg.chat.id, `Approved TX ${txId}`);
});
